...
export default function AlternateCaseClient() {
  const [input, setInput] = useState<string>('');
  const [output, setOutput] = useState<string>('');
  const [copySuccess, setCopySuccess] = useState(false);

  const convertToAlternateCase = (text: string): string => {
    let index = 0;
  ...
  useEffect(() => {
    if (input) {
      setOutput(convertToAlternateCase(input));
    } else {
      setOutput('');
    }
  }, [input]);
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = 'alternate-case-text.txt';
    a.click();
    ...
    <div className="space-y-6">
      <div className="bg-orange-50 border-l-4 border-orange-400 p-4 rounded-r-lg">
        <p className="text-sm text-orange-700">
          <strong>aLtErNaTe CaSe</strong> alternates between lowercase and uppercase. Famous as the SpongeBob mocking meme format.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="relative">
          ...
          <textarea
            ...
            className="w-full h-48 p-4 rounded-xl border-2 border-gray-200 bg-white text-gray-900 text-sm focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all resize-y placeholder:text-gray-300"
          />
          {input && (
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
              ...
              Output — <span className="text-orange-600">aLtErNaTe CaSe</span>
            </label>
            <div className="flex gap-2">
              {output && (
                <>
                  ...
                  <button onClick={handleCopy} className={`text-xs px-3 py-1.5 rounded-lg transition-all flex items-center gap-1 ${copySuccess ? 'bg-green-100 text-green-600' : 'bg-orange-50 text-orange-600 hover:bg-orange-100'}`}>
          ...
          <div className={`w-full h-48 p-4 rounded-xl border-2 transition-all overflow-y-auto ${output ? 'border-orange-400 bg-orange-50 text-gray-900' : 'border-gray-200 bg-gray-50 text-gray-400'}`}>
