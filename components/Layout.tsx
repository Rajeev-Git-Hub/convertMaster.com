...
const category = (pathSegment === 'length' || pathSegment === 'weight' || pathSegment === 'temperature' || pathSegment === 'data')
  ...
  const [collapsedSections, setCollapsedSections] = React.useState<Record<string, boolean>>({ 'converter': true });
      ...
      <div className="flex flex-1 overflow-x-hidden pt-16 md:pt-16">
        ...
        <aside className={`fixed left-0 top-16 md:top-16 w-64 h-[calc(100vh-4rem)] bg-gray-800 dark:bg-gray-950 border-r border-gray-700 dark:border-gray-800 overflow-y-auto custom-scrollbar z-50 ${
          ...
          <div className="p-2 md:p-6">
            ...
            ) : pathname?.includes('/unit-conversion') || pathname?.includes('/all-converters') ? (
              /* Unit Conversion Sidebar - Tools, Collapsible Converter */
              <>
                {/* Tools - Standalone */}
                <div className="mb-4">
                  <nav className="space-y-1">
                ...
                <div className="mb-8">
                  <button
                    ...
                    className="flex items-center justify-between w-full mb-4 text-sm text-gray-200 hover:text-white hover:bg-gray-700 rounded-lg transition-colors px-3 py-2"
                  >
                    <div className="flex items-center gap-3">
